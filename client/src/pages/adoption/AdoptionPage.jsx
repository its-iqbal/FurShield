    ...
    <div className="glass-card overflow-hidden group hover:scale-[1.02] transition-all duration-200 cursor-pointer"
      ...
      <div className="h-44 bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="absolute top-2 right-2">
          <span className={`text-xs font-bold px-2 py-1 rounded-full
            ${listing.status === 'available' ? 'bg-green-500/20 text-green-300 border border-green-500/30' : 'bg-gray-500/20 text-gray-400 border border-gray-500/30'}`}>
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {listing.age && <span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20">{listing.age}</span>}
          {listing.gender && <span className="text-xs px-2 py-0.5 rounded-full bg-white/5 text-gray-400 capitalize">{listing.gender}</span>}
          {listing.isVaccinated && <span className="text-xs px-2 py-0.5 rounded-full bg-green-500/10 text-green-400 border border-green-500/20">💉 Vaccinated</span>}
          {listing.isNeutered && <span className="text-xs px-2 py-0.5 rounded-full bg-purple-500/10 text-purple-400 border border-purple-500/20">✂️ Neutered</span>}
        </div>
        ...
        <div className="flex items-center justify-between">
          {listing.shelter?.shelterName && (
            <p className="text-gray-600 text-xs">🏠 {listing.shelter.shelterName}</p>
          )}
...
function ListingDetail({ listing, onSubmitInterest, isSubmitting, submitted }) {
  ...
  if (submitted) return (
    <div className="text-center py-8 animate-fade-in">
      ...
      <h3 className="text-xl font-black text-white mb-2">Interest Submitted!</h3>
      <p className="text-gray-400 text-sm">The shelter will review your request and contact you soon.</p>
    </div>
  );
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex items-center gap-4 mb-5">
        <div className="w-16 h-16 rounded-2xl bg-gray-800 flex items-center justify-center text-4xl border border-white/10 flex-shrink-0">
          {listing.images?.[0]
            ? <img src={listing.images[0]} alt={listing.name} className="w-full h-full object-cover rounded-2xl" />
            : emoji}
        </div>
        <div>
          <h2 className="text-2xl font-black text-white">{listing.name}</h2>
          <p className="text-gray-500 text-sm capitalize">{listing.species}{listing.breed ? ` · ${listing.breed}` : ''}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        {[
          ...
          <div key={i.label} className="bg-white/5 rounded-xl p-3 border border-white/10 text-center">
            <p className="text-gray-600 text-xs">{i.label}</p>
            <p className="text-white text-sm font-medium capitalize mt-0.5">{i.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {listing.isVaccinated && <span className="text-xs px-3 py-1 rounded-full bg-green-500/10 text-green-300 border border-green-500/20">💉 Vaccinated</span>}
        {listing.isNeutered   && <span className="text-xs px-3 py-1 rounded-full bg-purple-500/10 text-purple-300 border border-purple-500/20">✂️ Neutered</span>}
        {listing.isHouseTrained && <span className="text-xs px-3 py-1 rounded-full bg-blue-500/10 text-blue-300 border border-blue-500/20">🏠 House trained</span>}
      </div>

      {listing.description && (
        <div className="mb-5">
          <p className="text-xs text-gray-500 mb-2 uppercase tracking-wider font-medium">About {listing.name}</p>
          <p className="text-gray-300 text-sm leading-relaxed bg-white/5 rounded-xl p-4 border border-white/10">{listing.description}</p>
        </div>
      )}
      ...
      {listing.status === 'available' && (
        <div className="border-t border-white/10 pt-5">
          <h3 className="text-white font-bold mb-4">💌 Express Adoption Interest</h3>
  ...
  return (
    <DashboardLayout pageTitle="Adopt a Pet 🏠">
      <div className="px-4 sm:px-6 py-8 max-w-6xl mx-auto">
        <div className="mb-6">
          <h2 className="text-2xl font-black text-white">Find Your Companion 🐾</h2>
          <p className="text-gray-500 text-sm mt-1">Browse pets available for adoption from registered shelters</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-24">
            ...
            <p className="text-gray-400">No pets available right now. Check back soon!</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
