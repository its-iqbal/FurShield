  ...
  return (
    <div className="glass-card flex flex-col group hover:scale-[1.02] transition-all duration-200 overflow-hidden">
      ...
      <div className="h-44 bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center text-6xl
        border-b border-[#E8E2D9] overflow-hidden relative">
        {product.images?.[0]
          ? <img src={product.images[0]} alt={product.name} className="w-full h-full object-cover" />
          : <span className="opacity-30">🛍️</span>
        }
        {product.stock === 0 && (
          <div className="absolute inset-0 bg-page/70 flex items-center justify-center">
            <span className="text-xs font-bold text-[#8C4238] bg-red-500/20 border border-red-500/30 px-3 py-1 rounded-full">
              Out of Stock
            </span>
          </div>
        )}
        {product.discount > 0 && (
          <span className="absolute top-2 left-2 text-xs font-bold text-primary-600 bg-green-500/20
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex items-start gap-2 mb-1">
          <h3 className="text-strong font-bold text-sm flex-1 leading-snug line-clamp-2">{product.name}</h3>
        </div>
        <p className="text-muted text-xs mb-3 line-clamp-2 flex-1">{product.description}</p>

        <div className="flex flex-wrap gap-1.5 mb-3">
          <span className="text-xs px-2 py-0.5 rounded-full bg-primary-500/10 text-primary-700 border border-primary-500/20 capitalize font-medium">
            {product.category}
          </span>
          {product.petType && product.petType !== 'all' && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-primary-50 text-muted capitalize font-medium">
              {product.petType}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center justify-between mt-auto">
          <div>
            <span className="text-strong font-black text-lg">₹{product.price}</span>
            {product.discount > 0 && (
              <span className="text-subtle text-xs line-through ml-2">
          ...
          <button
            ...
            className={`text-xs font-semibold px-3 py-2 rounded-xl border transition-all duration-200
              ${added
                ? 'bg-green-500/20 border-green-500/40 text-primary-600'
        ...
        {product.stock > 0 && product.stock <= 5 && (
          <p className="text-xs text-accent-600 mt-2">⚠️ Only {product.stock} left!</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <DashboardLayout pageTitle="Pet Shop 🛒">
      <div className="px-4 sm:px-6 py-8 max-w-6xl mx-auto">
        ...
        {/* Order success toast */}
        {orderSuccess && (
          <div className="mb-5 p-4 rounded-xl bg-green-500/15 border border-green-500/30 text-primary-600 flex items-center gap-3 animate-fade-in">
            <span className="text-2xl">🎉</span>
            <div>
              <p className="font-semibold">Order placed successfully!</p>
              <p className="text-xs text-primary-600/70">We'll contact you shortly for delivery / pickup details.</p>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
          <div>
            <h2 className="text-2xl font-heading font-semibold text-strong">FurShield Shop</h2>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            ...
            className="flex-1 bg-white border border-[#E8E2D9] rounded-xl px-4 py-2.5 text-body
              placeholder-[#8A8279] text-sm focus:outline-none focus:border-primary-500 transition-all" />
          <select value={category} onChange={(e) => setCategory(e.target.value)}
            className="bg-white border border-[#E8E2D9] rounded-xl px-4 py-2.5 text-body
              text-sm focus:outline-none focus:border-primary-500 transition-all capitalize">
            {CATEGORIES.map((c) => <option key={c} value={c} className="bg-panel capitalize">{c === 'all' ? 'All Categories' : c}</option>)}
          </select>
          <select value={petType} onChange={(e) => setPetType(e.target.value)}
            className="bg-white border border-[#E8E2D9] rounded-xl px-4 py-2.5 text-body
              text-sm focus:outline-none focus:border-primary-500 transition-all capitalize">
            {PET_TYPES.map((t) => <option key={t} value={t} className="bg-panel capitalize">{t === 'all' ? 'All Pets' : t}</option>)}
          </select>
        </div>
