      ...
      <div className="h-40 bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center relative overflow-hidden">
        ...
        {article.mediaType === 'video' && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-12 h-12 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-body text-2xl">▶</div>
          </div>
        )}
        <div className="absolute bottom-2 left-2 flex gap-1.5">
          <span className="text-xs px-2 py-0.5 rounded-full bg-panel/80 text-body capitalize">{catEmoji} {article.category}</span>
          {article.petType && article.petType !== 'all' && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-primary-500/50 text-body capitalize">{article.petType}</span>
          )}
        </div>
      </div>
      <div className="p-4">
        <h3 className="text-strong font-bold text-sm line-clamp-2 mb-2 leading-snug">{article.title}</h3>
        {article.summary && <p className="text-muted text-xs line-clamp-2 mb-3">{article.summary}</p>}
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        {article.petType && article.petType !== 'all' && (
          <span className="text-xs px-2.5 py-1 rounded-full bg-primary-50 text-muted capitalize">{article.petType}</span>
        )}
        {article.mediaType && article.mediaType !== 'article' && (
          <span className="text-xs px-2.5 py-1 rounded-full bg-[#E8F0F5] text-info-500 capitalize">{article.mediaType}</span>
        )}
      </div>

      <h2 className="text-xl font-black text-body mb-2">{article.title}</h2>
      ...
      {article.summary && (
        <div className="p-4 rounded-xl bg-primary-500/5 border border-primary-500/15 mb-5">
          <p className="text-body text-sm italic">{article.summary}</p>
        </div>
      )}
      ...
      {article.videoUrl && (
        <div className="mb-5">
          <a href={article.videoUrl} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-3 p-4 rounded-xl bg-[#F4EBE8] border border-[#E0C8C4] text-[#8C4238] hover:bg-red-500/15 transition-all">
      ...
      {/* Content */}
      {article.content && (
        <div className="prose prose-invert prose-sm max-w-none text-body leading-relaxed text-sm whitespace-pre-wrap mb-5">
          {article.content}
        </div>
      )}
      ...
      {article.tags?.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-5">
          {article.tags.map((t) => (
            <span key={t} className="text-xs px-2.5 py-1 rounded-full bg-primary-50 text-muted border border-[#E8E2D9]">#{t}</span>
          ))}
        </div>
      )}
      ...
      <div className="flex items-center justify-between pt-4 border-t border-[#E8E2D9]">
        <button onClick={() => onLike(article._id)} id={`like-article-${article._id}`}
          className="flex items-center gap-2 text-sm text-muted hover:text-[#8C4238] transition-colors">
  ...
  return (
    <DashboardLayout pageTitle="Care Tips 📰">
      <div className="px-4 sm:px-6 py-8 max-w-6xl mx-auto">
        <div className="mb-6">
          <h2 className="text-2xl font-heading font-semibold text-strong">Pet Care Library</h2>
          <p className="text-muted text-sm mt-1">Expert articles, videos, and guides for every pet parent</p>
        </div>

        {/* Filters */}
        <div className="flex flex-wrap gap-3 mb-6">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="🔍 Search tips…"
            className="bg-white border border-[#E8E2D9] rounded-xl px-4 py-2 text-body placeholder-[#8A8279] text-sm focus:outline-none focus:border-primary-500 transition-all" />
          {[
            ...
            <select key={label} value={val} onChange={(e) => set(e.target.value)}
              className="bg-white border border-[#E8E2D9] rounded-xl px-4 py-2 text-body text-sm focus:outline-none focus:border-primary-500 transition-all capitalize">
              {opts.map((o) => <option key={o} value={o} className="bg-panel capitalize">{o === 'all' ? `All ${label}s` : o}</option>)}
            </select>
          ))}
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="glass-card animate-pulse">
                <div className="h-40 bg-primary-50 rounded-t-2xl" />
          ...
          <div className="text-center py-24">
            ...
            <p className="text-muted">No articles found. Try different filters.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
