...
const vetLinks = [
  ...
  { icon: '🐾', label: 'Patient Records',  color: 'from-info-400/20 border-blue-500/30' },
  { icon: '👤', label: 'My Profile',       color: 'from-info-400/20 border-info-400' },
    ...
    <div className="min-h-screen bg-page text-body">
      <header className="border-b border-[#E8E2D9] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-2 text-xs px-2 py-0.5 rounded-full bg-blue-500/20 text-info-500 border border-blue-500/30">
            Veterinarian
          </span>
        </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        <p className="text-muted text-sm mb-1">Veterinarian Portal</p>
        <h1 className="text-4xl font-black text-body mb-2">
          Dr. <span className="gradient-text">{user?.name}</span>
        </h1>
        {user?.specialization && (
          <p className="text-muted mb-8">🩺 {user.specialization}</p>
        )}
        ...
        <div className="mt-12 glass-card p-6 text-center">
          <p className="text-muted text-sm">🚧 Veterinarian features are being built. Stay tuned!</p>
        </div>
      </main>
    </div>
  );
